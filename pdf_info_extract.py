...
def extract_content_by_keyword(text, field_name):
    ...
    # 定义关键词映射
    keyword_mapping = {
        "质量保证方案": ["质量保证","质量目标","质量管理","质量控制"],
        "安全保障": ["安全保障", "安全措施","安全生产责任制","安全教育培训"],
        "进度管理方案": ["进度管理", "进度控制", "进度计划","工期保证"],
        "应急预案": ["应急预案", "应急处理", "应急措施", "突发事件", "应急响应"],
        "服务承诺": ["服务承诺", "服务保证","售后服务","保修承诺","满意"],
        "施工标准": ["施工标准", "施工规范" , "技术标准","GB/T","ISO","标准化","BIM"],
        "巡查考核方案": ["组织巡查", "监督考核","定期检查","第三方抽检"],
        "沟通配合措施": ["沟通配合", "沟通机制","联络人","信息通报","问题反馈","争议解决","业主","利益相关"],
        "安全文明建设": ["安全文明", "文明施工","场容场貌","环境保护","绿色","节能","环保"],
        "制度建设": ["制度建设","管理流程","内部审核","信息化管理","内部评审"],
        "资料整编方案": [ "资料管理","电子化管理","审核流程","资料规范","资料移交"]
    }
...
def determine_file_type(file_name, text_preview=""):
    ...
    if any(keyword in file_name_lower for keyword in ["招标", "采购", "公告"]):
        return "招标文件"
    elif any(keyword in file_name_lower for keyword in ["投标", "响应", "报价" , "技术部分"]):
        return "投标文件"
